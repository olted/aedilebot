...
class EntityNotFoundError(Exception):
    def __init__(
        self,
        name,
        message="The requested entity was not found. Please try again.",
    ):
        self.name = name
...
class InvalidTypeError(EntityNotFoundError):
    def __init__(
        self,
        name,
        message="The entity was invalid for this operation. Please try again.",
    ):
...
class TargetNotFoundError(EntityNotFoundError):
    def __init__(
        self,
        name,
        message=f"The detected target is not found in my data set. Please try again.",
    ):
        super().__init__(name, message)
...
class TargetOfTypeNotFoundError(EntityNotFoundError):
    def __init__(
        self,
        name,
        message=f"The detected target for this operation is not found in my data set. Please try again.",
    ):
        super().__init__(name, message)
...
class WeaponNotFoundError(EntityNotFoundError):
    def __init__(
        self,
        name,
        message=f"The detected weapon is not found in my data set. Please try again.",
    ):
        super().__init__(name, message)
...
class LocationNotFoundError(EntityNotFoundError):
    def __init__(
        self,
        name,
        message="I could not process a request because the town/relic was not found. Please try again.",
    ):
        super().__init__(name, message)
...
def run_discord_bot():
    ...
    client = main.commands.Bot(
        command_prefix="!", intents=intents, proxy=HTTPS_PROXY
    )
    ...
    async def help(interaction: discord.Interaction):
        embed = discord.Embed(
            title="__**Help and Commands**__",
            description="Welcome to the help section. Here you will be provided with a description of how the bot works and its commands.\n",
            color=0x992D22,
            timestamp=datetime.datetime.utcnow(),
        )
        embed.add_field(
            name="Version",
            value="0.8.61 (Up to date with foxhole U61)",
            inline=False,
        )
        embed.set_thumbnail(
            url="https://media.discordapp.net/attachments/884587111624368158/1077553561010982922/g839.png?width=570&height=498"
        )
        embed.add_field(
            name="/help",
            value="Provides you with this message! How neat is that?",
            inline=False,
        )
        embed.add_field(
            name="/statsheet [entity]",
            value="Provides you with a statistics sheet of any entity in the calculator",
            inline=False,
        )
        embed.add_field(
            name="/kill [target] [weapon]",
            value="Fulfills the same purpose as the prompt below with the help of autocomplete.",
            inline=False,
        )
        embed.add_field(
            name="/bunker_kill",
            value="Specify the bunker island you want to kill with detailed parameters. (If a modification is not an option, it means it has the same stat as a blank bunker such as engine room)",
            inline=False,
        )
        embed.add_field(
            name="**Damage Calculator Prompt**",
            value="``How much|many [weapon] to destroy|kill|disable [target]``\nHere are some examples to try:\n\nHow much 150mm to kill Patridia?\nHow many satchels to kill t3 bunker core husk?\nHow many 68mm to disable HTD?\nHow many satchels to kill Victa?\nHow much 40mm to destroy bt pad?",
            inline=False,
        )
        embed.set_footer(
            text="Good luck on the front!",
            icon_url="https://media.discordapp.net/attachments/884587111624368158/1077553561010982922/g839.png?width=570&height=498",
        )
    ...
    async def custom_kill(
        ...
        await interaction.response.send_message(
            handle_response_inner(
                weapon1, target, query, "custom_kill", num1, weapon2
            )
        )
    ...
    async def statsheet(interaction: discord.Interaction, entity: str):
        ...
        if data[0] == "Weapons":
            ...
            if data[6] != "0":
                ...
                embed.add_field(
                    name="Armour Health", value=data[6], inline=True
                )
            if data[7] != "":
            ...
            if data[9] != "":
                embed.add_field(
                    name="Turret Disable Chance",
                    value=str(data[9]) + "%",
                    inline=True,
                )
            if data[10] != "":
                embed.add_field(
                    name="Tracks Disable Chance",
                    value=str(data[10]) + "%",
                    inline=True,
                )
        ...
        if len(current) > 1:
            ...
            for possible_value in guess:
                if possible_value.lower() != possible_value and len(data) < 20:
                    ...
                    possible_value.lower() == possible_value and len(data) < 20
                ):
    ...
    async def kill_autocompletion(
        ...
        value = [
            fuzz.fuzzy_match_target_name(v) for v in guess if v.lower() == v
        ]
...
# bot logic
def handle_response_inner(
    weapon, target, query=None, operation="kill", num1=0, weapon2=None
):
    try:
        if operation == "kill":
            return calculator.general_kill_handler(
                weapon, target
        ...
        if operation == "custom_kill":
            return calculator.custom_kill_handler(
                weapon, num1, weapon2, target
            )
        ...
        return (
            "Inner error happened during processing of your request. "
...
def handle_bunker_kill_command(
    weapon,
    total_size,
    tier,
    green_dots,
...
def handle_response(message_) -> str:
    ...
    if len(token_pair) >= 1:
        ...
        if "size" in target:
            return handle_response_inner(
                weapon, target, message_, operation="bunker"
            )
    ...
    if len(token_pair) >= 1:
        ...
        return handle_response_inner(
            weapon, target, message_, operation="disable"
        )
    ...
    if len(token_pair) >= 1:
        weapon, target = token_pair[0][1], token_pair[0][2]
        return handle_response_inner(
            weapon, target, message_, operation="dehusk"
        )
