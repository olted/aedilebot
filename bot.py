...
class StructureNotFound(Exception):
    "Structure text something"
    pass


class WeaponNotFound(Exception):
    "Weapon text something"
    pass
...
def fuzzy_match_th_relic_name(name):
    ...
    if max_score < 90:
        ...
        return False, "Target name not found. Try again with the full name of your target."
    return True, max_key
...
def general_h2k_handler(weapon_fuzzy_name, target_fuzzy_name):
    weapon_result, weapon_name = fuzzy_match_weapon_name(weapon_fuzzy_name)
    structure_result, structure_name = fuzzy_match_structure_name(target_fuzzy_name)
    if not weapon_result:
        return False, weapon_name
    if not structure_result:
        return False, structure_name
    data = damage_calculator(weapon_name, structure_name)

    return True, f"It takes {data['htk']} {weapon_name} to kill a {structure_name}"


def relic_th_h2k_handler(weapon_fuzzy_name, location_fuzzy_name):
    fuzzy_result, location_name = fuzzy_match_th_relic_name(location_fuzzy_name)

    if not fuzzy_result:
        return False, location_name

    fuzzy_result, weapon_name = fuzzy_match_weapon_name(weapon_fuzzy_name)
    ...
    if "relic" in target_name:
        return True, output_string + f"{damage_calculator(weapon_name, target_name)['htk']}"
    ...
    if weapons_dict[weapon_name]["DamageType"] == "BPDemolitionDamageType":
        ...
        return True, output_string + f"{damage_calculator(weapon_name, target_name_tiered)['htk']}"

    t = []
    for tier in ["T1", "T2", "T3"]:
        t.append(damage_calculator(weapon_name, f"{target_name} {tier}")['htk'])
    return True, output_string + f"{t[0]} (Tier 1) {t[1]} (Tier 2) {t[2]} (Tier 3)"
...
def handle_response_inner(message_):
    ...
    if len(token_pair) >= 1:
        ...
        static_result, static_result_text = relic_th_h2k_handler(weapon, target)

        if static_result:
            return static_result_text

        dynamic_result, dynamic_result_text = general_h2k_handler(weapon, target)

        return dynamic_result_text
    return ""
